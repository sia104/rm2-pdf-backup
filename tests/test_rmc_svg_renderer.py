...
def test_render_svg_pages_accepts_nonempty_svg_with_nonzero_exit(tmp_path: Path) -> None:
    raw = tmp_path / "raw"
    doc_dir = raw / "doc"
...
def test_render_reports_missing_pdf_composer_after_svg_success(tmp_path: Path) -> None:
    raw = tmp_path / "raw"
    doc_dir = raw / "doc"
    doc_dir.mkdir(parents=True)
    ...
    assert "PDF composition" in result.error
